...
> **注意**：`windows-x86` 必须使用 x86 版 IFW 生成安装器。IFW 4.5.2 没有官方 x86 预编译包，因此使用最新可用的 IFW 4.3.0 x86。IFW 4.3.0 x86 不支持 `PersistentLocalCache`，因此 x86 分支使用临时生成的 `config/config-x86.xml`，并通过 PE 头校验确认安装器 `Machine` 为 `0x14c`。
